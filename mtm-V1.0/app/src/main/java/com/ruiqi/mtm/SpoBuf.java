...
public class SpoBuf {
    ...
    public List<byte[]> mSp02DataList = new ArrayList();
    ...
    public synchronized void write(byte[] buf, int count,
        ...
        switch (_receiveNum) {
            ...
            case 2:// 对时成功
                try {
                    pOutputStream.write(DeviceCommand.setPedometerInfo("175", "75", 0, 24, 10000, 1,2));
                    Log.i(TAG, "校时成功 发送设置计步器信息");
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
            ...
            case 5: // 整个数据接收完成
                ...
                //字节数组的长度是8，前6个字节表示血氧、脉率的测量时间： 年 （eg：2014->14）、月、日、时、分、秒，
                // 第7、8个字节分别表示血氧值和脉率值。
                DeviceData50DJ_Jar _djData = m_DevicePackManager.getDeviceData50dj();
                Log.i(TAG,"数据0："+_djData.toString());
                Log.i(TAG,"数据："+_djData.getmSp02DataList().toString());
                for (int i = 0; i < _djData.getmSp02DataList().size(); i++) {
                    ...
                    msg2.obj = data;
                    Log.v(TAG,"time:"+date+"血氧:"+sp0+"/脉率"+bpm);
                    myHandler.sendMessage(msg2);
                }
                mSp02DataList.clear();
                _djData.setmSp02DataList(mSp02DataList);
                //上传完成
                //发送请求计步器以天为单位的数据的命令
                pOutputStream.write(DeviceCommand.dayPedometerDataCommand());
                Log.v(TAG,"发送以天为单位的计步器数据");
            ...
            case 6://一包数据接收完毕 ；发送数据上传成功的指令，（必须这样发）
                Log.i(TAG, "一包血氧、脉率数据接收完毕");
                Log.i(TAG, ""+m_DevicePackManager.getDeviceData50dj());
                pOutputStream.write(DeviceCommand.dataUploadSuccessCommand ());
                break;
            case 7:// 接收失败
                Log.i(TAG, "血氧数据接收失败  请求以天为单位的计步器数据");
                pOutputStream.write(DeviceCommand.dayPedometerDataCommand());
                break;
            case 8:// 8:设置计步器 成功
                try {
                    pOutputStream.write(DeviceCommand.getDataFromDevice());
                    Log.i(TAG, "设置计步器 成功  发送请求血氧数据命令");
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
            ...
            case 18:// 以分为单位 计步器无新数据
                ...
                Message msg3 = myHandler.obtainMessage(BluetoothService.STATE_BROKEN);
